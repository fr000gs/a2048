pub mod game {
    ...
    impl Board {
        ...
        fn move_direction(&mut self, start: usize, end: usize, row_or_col: bool) {
            ...
            for i in start..end {
                ...
                while current != end && self.grid[row_or_col as usize][current] == 0 {
                    self.grid[row_or_col as usize][current] = self.grid[row_or_col as usize][current + (end - start) as usize];
                    self.grid[row_or_col as usize][current + (end - start) as usize] = 0;
                    current += (end - start) as usize;
                    moved = true;
                }

                if current != end && self.grid[row_or_col as usize][current] == self.grid[row_or_col as usize][current + (end - start) as usize] {
        ...
        pub fn get_board(&self) -> &[[u32; BOARD_SIZE]; BOARD_SIZE] {
            &self.grid
        }
    }
}
